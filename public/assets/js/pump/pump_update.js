...
verify();
  ...
  let verifyData =  JSON.parse(PUMP_ID.verified_place);

  let parse = JSON.parse(PUMP_ID.isolation);
  ...
  let signature = {
    'inspector' : PUMP_ID.inspector,
    'place' : PUMP_ID.place,
    'date' : PUMP_ID.date,
  };
  
document.querySelector(`input[data-signify="inspector"]`).value = PUMP_ID.inspector == 'null' ? '' : PUMP_ID.inspector;
document.querySelector(`input[data-signify="place"]`).value = PUMP_ID.place == 'null' ? '' : PUMP_ID.place;
document.querySelector(`input[data-signify="date"]`).value = PUMP_ID.date == 'null' ? '' : PUMP_ID.date;
...
const isolationsData =  PUMP_ID ? JSON.parse(PUMP_ID.isolation) : [];
...
document.querySelector(`[id-output="output"]`)
    .style.backgroundImage = `url(${PUMP_ID.signatures})`
      ...
      <div class="row mt-3">
        <div class="col-md-6">
          <div class="form-floating">
            <input type="text" class="form-control" id="floatingItem" placeholder="Item" value="${PUMP_ID.item}" disabled>
            <label for="floatingItem">Item</label>
          </div>
        </div>
        <div class="col-md-6">
          <div class="form-floating">
            <input type="txt" class="form-control" id="floatingSerial" placeholder="Serial No." value="${PUMP_ID.serial_no}" disabled>
  ...
  document.getElementById('updateButton').addEventListener('click', async (e) => {
      ...
      form.append('serial_no', PUMP_ID.serial_no);
      form.append('item', PUMP_ID.item);
